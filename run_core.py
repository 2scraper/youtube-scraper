#!/usr/bin/env python3
"""run_core.py — the run, once, for every transport.

Every engine in this repo used to carry its own copy of what follows.
Measured on 2026-09-25, the day before this module existed: of the 29
definitions the three engines shared, **24 were byte-identical** — 1,078
lines each, 2,156 of them duplicated — and the five that differed were
exactly the five that touch a browser library. `scrape()` differed by one
line, the engine's own name.

So this is a move, not a rewrite, and it is CLAUDE.md §26's pattern
(binance-scraper: one fetch loop, three drivers) applied to a repo whose
duplication had already been measured and left in place.

Two things follow, and both were the point:

* **The HTTP transport no longer needs a browser library to exist.**
  `playwright_scraper.py` imports Playwright at module level — §10
  requires that, so the suite's skip and CI's import check keep working —
  and it used to be the only CLI, including for the path that opens no
  browser. A reader following this repo's own quick start, which installs
  `requirements.txt` and no engine, met `ModuleNotFoundError: No module
  named 'playwright'` on their first command. `youtube_scraper.py`
  imports this module and no driver.
* **A fix lands once.** Four of the 2026-09-25 audit's findings live in
  the code below; before this module they were twelve edits with three
  chances to disagree.

The driver interface is deliberately tiny and was DERIVED rather than
designed: exactly four members, because exactly four names in the shared
code reached into a browser.

    name                      "playwright" | "selenium" | "puppeteer"
    context()                 the driver's own lifetime as a context
                              manager, yielding the driver with its
                              handle bound
    launch_local(args, pool)  -> a session
    connect_remote(args)      -> a session

No JavaScript crosses that boundary (CLAUDE.md §1): Selenium's
`execute_script` takes a function BODY with an explicit return while the
other two take an arrow expression, so a shared module that passed JS
...
# "The driver failed" is a property of the SESSION here, not a module
# constant: each session carries `errors`, the tuple its own library
# raises. A module-level binding would have been written by whichever
# engine imported last, which is the kind of quiet global this extraction
# exists to remove.
...
def handle_captcha_if_present(session, args, budget: SolveBudget) -> bool:
    ...
    The annotation on this used to promise a browser session, which
...
def _call(session, args, body: Dict[str, Any],
...
def parse_args(argv: Optional[List[str]] = None):
    ...
    p.add_argument("--engine", choices=("playwright", "selenium", "puppeteer"),
                   default="playwright",
                   help="Which browser engine youtube_scraper.py should "
                        "reach for when one is actually needed. Ignored by "
                        "the three engine scripts, which are already one. "
                        "The library is imported only if a browser is "
                        "opened, so naming an engine you have not installed "
...
class Driver:
    """What the loop above needs from a browser library, and nothing else.

    Four members, and the number is not a design choice: it is how many
    names in the shared code reached into a browser when this module was
    split out of `playwright_scraper.py` on 2026-09-25.
    """

    name = "none"

    def context(self):
        """The driver's own lifetime. Yields the driver with its handle
        bound, so a worker that opens its own can do so the same way."""
        return self
...
class NoDriver(Driver):
    """The HTTP transport's driver: it starts nothing.

    `youtube_scraper.py` runs with this, which is what lets a core-only
    install work. Asking it for a browser names the engine to install
    rather than raising ImportError from somewhere inside a run — the
    reader is one `pip install` from what they wanted, and an
    ImportError's text does not say so.
    """
    ...
    def _refuse(self, why):
        raise ProxyError(
            "%s, and no browser engine is installed. Install one:\n"
            "    pip install -r requirements-playwright.txt "
            "&& playwright install chromium\n"
            "then run playwright_scraper.py (or selenium_/puppeteer_) "
            "instead of youtube_scraper.py." % why)

    def launch_local(self, args, pool):
        self._refuse("This run needs a browser")

    def connect_remote(self, args):
        self._refuse("--cdp-endpoint needs a browser engine")


def main(driver, argv=None) -> int:
    """The CLI, shared. Each engine's `__main__` is three lines around it."""
