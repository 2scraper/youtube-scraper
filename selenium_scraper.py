...
# Every remote call is bounded (CLAUDE.md §8). Selenium's default script
# timeout is 30 s and its page-load timeout is unbounded, so both are set
# explicitly on every call rather than inherited.
...
if __name__ == "__main__":
    ...
    try:
        ...
        # A remote browser this driver cannot reach is a REMOTE API
        # failure (exit 5), not a crash in this code (exit 1) and not bad
        # usage (exit 2) — the flag is valid, this engine just cannot
        # honour it.
